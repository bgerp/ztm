...
#region Public Methods
    ...
    def update(self):
        ...
        ac_zone_occupied_flag = 0
        ac_zone_occupied = self._registers.by_name("ac.zone_occupied")
        if ac_zone_occupied is not None:
            ac_zone_occupied_flag = ac_zone_occupied.value

        # If the window is opened, just turn off the HVAC.
        window_closed_1_state = False
        gpio = self._registers.by_name("ac.window_closed_1.input")
        if gpio is not None:
            window_closed_1_state = self._controller.digital_read(gpio.value)
            self._registers.write("ac.window_closed_1.state", window_closed_1_state)

        stop_flag = (ac_zone_occupied_flag or not window_closed_1_state)
        ...
        if self.__update_timer.expired and self.__stop_flag == 0:
